#!/usr/bin/env bash
# bootstrap-container.sh — Build a Pleiades Gentoo systemd-nspawn root.
#
# Default behavior is intentionally narrow:
#   1. validate or create a marked container root;
#   2. verify a Gentoo stage3 before extraction;
#   3. install one commit-pinned Pleiades script snapshot with a content receipt.
#
# Host systemd and WSL boot changes are explicit opt-ins. The script never starts
...
Options:
  ...
  --adopt-existing-root       Mark a reviewed existing Linux root as Pleiades
  --update-scripts            Replace an older or altered script snapshot
  ...
  --stage3-mirror URL         Gentoo stage3 mirror (HTTPS required)
  --stage3-sha512 HASH        Independently pin the stage3 SHA-512
...
if [[ -n "${PLEIADES_TERMUX_LIB:-}" ]]; then
    [[ -r "$PLEIADES_TERMUX_LIB" ]] \
        || die "PLEIADES_TERMUX_LIB is not readable: $PLEIADES_TERMUX_LIB"
    # shellcheck source=/dev/null
    source "$PLEIADES_TERMUX_LIB"
fi

if [[ "${PLEIADES_ENV:-}" == "termux" ]]; then
    log "Termux detected; systemd-nspawn bootstrap is not applicable"
    log "Use the Pleiades Termux adapter"
    exit 0
fi
...
if $DRY_RUN; then
    log "Dry run: write, privilege, network, and archive checks are not executed"
else
    [[ "${EUID:-$(id -u)}" -eq 0 ]] || die "run as root (or use --dry-run)"
    for cmd in curl git mktemp rsync sha256sum sha512sum systemd-detect-virt systemd-nspawn tar; do
...
TMP_WSL=""
cleanup() {
    [[ -z "$TMP_WORK" || ! -d "$TMP_WORK" ]] || rm -rf -- "$TMP_WORK"
    [[ -z "$STAGING_ROOT" || ! -d "$STAGING_ROOT" ]] || rm -rf -- "$STAGING_ROOT"
    [[ -z "$TMP_WSL" || ! -e "$TMP_WSL" ]] || rm -f -- "$TMP_WSL"
}
...
install_stage3() {
    local parent metadata stage3_path stage3_url archive digests filename expected listing

    parent="$(dirname "$CONTAINER_ROOT")"
    if $DRY_RUN; then
        run install -d -m 0755 "$parent"
        log "Would resolve the latest Gentoo systemd stage3 from $STAGE3_MIRROR"
        log "Would verify SHA-512, inspect archive paths, extract to a sibling staging root, mark it, then atomically rename"
        return
    fi
    ...
    [[ "$stage3_path" != /* && "$stage3_path" != *'..'* ]] \
        || die "unsafe stage3 path in mirror metadata: $stage3_path"
    ...
    [[ ! -e "$CONTAINER_ROOT" ]] \
        || die "container root appeared during staging: $CONTAINER_ROOT"
...
if [[ -d "$CONTAINER_ROOT/usr" ]]; then
    ...
    [[ ! -e "$CONTAINER_ROOT" ]] \
        || die "container root exists but has no usr directory: $CONTAINER_ROOT"
    install_stage3
fi

install_scripts() {
    local clone_dir source_dir scripts_dir receipt
    local current_commit expected_tree actual_tree resolved_commit installed_tree

    scripts_dir="$CONTAINER_ROOT/scripts"
    receipt="$scripts_dir/.pleiades-source-receipt"

    if $DRY_RUN; then
        log "Would fetch $PLEIADES_REPO ref $PLEIADES_REF and resolve exactly one commit"
        log "Would install root.x86_64/scripts and record both commit and content-tree SHA-256"
        return
    fi
    ...
    if [[ -f "$receipt" ]]; then
        ...
        expected_tree="$(awk -F= '$1 == "content_tree_sha256" {print $2; exit}' "$receipt")"
    fi

    if [[ -d "$scripts_dir" && -n "$current_commit" && "$current_commit" == "$resolved_commit" ]]; then
        actual_tree="$(hash_tree "$scripts_dir")"
        if [[ -n "$expected_tree" && "$actual_tree" == "$expected_tree" ]]; then
            log "Pleiades scripts match pinned commit and content receipt: $resolved_commit"
            return
        fi
        $UPDATE_SCRIPTS \
            || die "script receipt commit matches but content drifted; review and use --update-scripts"
    elif [[ -d "$scripts_dir" && -n "$(find "$scripts_dir" -mindepth 1 -print -quit 2>/dev/null)" ]]; then
        $UPDATE_SCRIPTS \
            || die "scripts already exist without the requested pinned receipt; review and use --update-scripts"
    fi

    install -d -m 0755 "$scripts_dir"
    rsync -a --checksum --delete-delay --exclude '.git/' "$source_dir/" "$scripts_dir/"
    installed_tree="$(hash_tree "$scripts_dir")"
    {
        printf 'schema=1\n'
        printf 'source_repository=%s\n' "$PLEIADES_REPO"
        printf 'source_ref=%s\n' "$PLEIADES_REF"
        printf 'source_commit=%s\n' "$resolved_commit"
        printf 'content_tree_sha256=%s\n' "$installed_tree"
    } > "$receipt"
    log "Installed Pleiades scripts pinned to $resolved_commit with tree hash $installed_tree"
}
...
install_host_services() {
    ...
    $INSTALL_HOST_SERVICES \
        || { log "Host systemd units not installed (use --install-host-services)"; return; }
    ...
    command -v systemctl >/dev/null 2>&1 \
        || die "systemctl is required for --install-host-services"
    for svc in "$SCRIPT_DIR"/systemd/system/*.service; do
        [[ -e "$svc" ]] || continue
        name="$(basename "$svc")"
        ...
        if [[ -e "$dest" ]]; then
            cmp -s "$svc" "$dest" \
                || die "refusing to overwrite differing host unit: $dest"
...
install_wsl_boot() {
    ...
    $INSTALL_WSL_BOOT \
        || { log "WSL boot entry not installed (use --install-wsl-boot on WSL)"; return; }
    [[ "$ENVIRONMENT" == "wsl" ]] || die "--install-wsl-boot is valid only in WSL"

    if $DRY_RUN; then
        log "Would require executable $wrapper, back up $conf, and replace only the [boot] command"
        return
    fi

    [[ -x "$wrapper" ]] \
        || die "WSL runtime wrapper is missing or not executable: $wrapper"
    TMP_WSL="$(mktemp "${TMPDIR:-/tmp}/wsl.conf.XXXXXX")"
    ...
    awk -v cmd="$wrapper" '
        ...
        { print }
        END {
            if (in_boot && !wrote_command) { print "command=" cmd }
            if (!found_boot) { print ""; print "[boot]"; print "command=" cmd }
        }
    ' "$conf" > "$TMP_WSL"
    install -m 0644 "$TMP_WSL" "$conf"
    rm -f -- "$TMP_WSL"
    TMP_WSL=""
...
log "No host service was enabled or started."
log "Next steps:"
log "  1. Review $CONTAINER_ROOT/scripts/.pleiades-source-receipt"
log "  2. Configure the operator inside the container"
log "  3. Install host units explicitly if desired"
log "  4. Start and inspect the container through reviewed install scripts"
